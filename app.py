...
# 1. Page Config & CSS for a Clean Look
...
with st.sidebar:
    ...
    dev_name = st.text_input("Development", "Sample Project")
    unit_no  = st.text_input("Unit Number", "12-34")
    sqft     = st.number_input("Size (sqft)", value=1000)
    u_type   = st.text_input("Unit Type", "3-Bedroom")
    
    st.divider()
    st.title("💰 Pricing Data")
    fmv    = st.number_input("Fair Market Value (PSF)", value=2000)
    my_ask = st.number_input("Your Asking (PSF)", value=2100)
    
    st.divider()
    st.title("📊 Market Range")
    t_low  = st.number_input("Min Transacted PSF", value=1900)
    t_high = st.number_input("Max Transacted PSF", value=2150)
    a_low  = st.number_input("Min Asking PSF", value=2000)
    a_high = st.sidebar.number_input("Max Asking PSF", value=2300)
...
ax.axvspan(lower_5, upper_5, color='#2ecc71', alpha=0.12, label="Fair Value")
