...
# --- CSS: FORCE BLACK TEXT & SIDEBAR VISIBILITY ---
st.markdown("""
    <style>
...
with st.sidebar:
    ...
    fmv, our_ask = st.number_input("Fair Market Value", value=1050), st.number_input("Our Asking", value=1100)
...
if has_data:
    ...
    status_text, status_color, diff_pct = "Input Data", "#7f8c8d", 0
...
if has_data:
    ...
    fig.subplots_adjust(top=0.85, bottom=0.15) # Leave space for header and footer labels
    ...
    ax.text((data_min + data_max)/2, 3.1, f"STATUS: {status_text}", fontsize=26, weight='bold', color=status_color, ha='center')

    # --- CENTERED ALIGNMENT: BAR & LOGO ---
    ...
    # Horizontal Details Bar: Aligned with the MIDDLE of the logo (y=0.92)
